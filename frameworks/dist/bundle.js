...
(function () {
    ...
    !function () {
        ...
        function creat(id, scale, tdpi) {
            document.write('<meta ' + 'id="' + id + '" ' + 'name="viewport" ' + 'content="' + 'width=device-width,' + 'initial-scale=' + scale + ',' + 'minimum-scale=' + scale + ',' + 'maximum-scale=' + scale + ',' + 'user-scalable=no' + (tdpi ? devicePixelRatio > 2 ? '' : ',target-densitydpi=device-dpi' : '') + '">');
        ...
        var scale = window.innerWidth !== window.screen.width && windowRestWidth !== windowInitWidth ? Math.max(Math.round(window.innerWidth / windowRestWidth), Math.round(document.documentElement.offsetWidth / documentElementInitWidth)) : null;
        ...
        Module.prototype = {
            get: function (name, callback, faild) {
                return callback ? define('argument', name, function (require) {
                    callback(require);
                }, faild) : modules[name] ? modules[name].exports : null;
            },
            create: function () {
    ...
    var DETECT = function (userAgent) {
        ...
        this.prefixStyle = function (prop, css) {
            ...
            for (var i = 0, l = VENDORS.length; i < l; i++) {
                api = VENDORS[i] + ('-' + prop).replace(/-(\w)/g, function () {
        ...
        this.getPrefixStyleProp = function (prop) {
            ...
            for (var i = 0, l = VENDORS.length; i < l; i++) {
                var prefix = VENDORS[i] + ('-' + prop).replace(/-(\w)/g, function (context, word) {
        ...
        this.supportSizeCalc = function () {
            var initialDocumentWidth, supportSizeCalc;
                            ...
                            switch (this.getInstanceType()) {
                                ...
                                default:
                                    ...
                                    for (i = 0; i < this.length; i++) {
                                        if (callback.call(that, i, this[i], i + 1, this.length) === false) return this;
                                    }
                ...
                !function (proto) {
                    ...
                    if (!proto.set) {
                        ...
                        proto.set = function (propertyName, value) {
                            ...
                            value = typeof value == 'string' ? value.replace(REGEXP.size, function (size, length, unit) {
