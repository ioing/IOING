var datePicker = function(that, options) {
    return that.each(function(e) {
        ...
        if ( !opts.show ) {
            ...
            // new Touch(that).on('tap', function (event) {
            //     init()
        ...
        function init() { //初始化函数
            ...
            // node('date-plugin').addEventListener('touchstart', function (event) {
            ...
            new Touch(root.find('#d-okBtn')[0]).on('tap', function(event) {
                ...
                setTimeout(function () {
                    destroyScroll()
                    // node('date-plugin').removeEventListener('touchmove', cancleDefault, false)

                    // var y = root.find('#yearScroll li').eq(initY).data('num');
                    // var M = root.find('#monthScroll li').eq(initM).data('num');
                    // var d = root.find('#dayScroll li').eq(initD).data('num');
                    // var h = root.find('#hourScroll li').eq(initH).data('num');
                    // var m = root.find('#minuteScroll li').eq(initI).data('num');
                    //that.val(root.find('.d-return-info').html());
        ...
        function renderDom() {
            ...
            switch (opts.theme) {
                ...
                case 'time':
                    ...
                    break
                case 'month':
                    node('d-date-box').appendTemplate("mouth", {})
                    createYear()
                    createMonth()
                    break
                default:
        ...
        function createHour() {
            var hourDom = root.find('#hourScroll'),
                hourHtml = '<li></li>';
            ...
            hourScroll = new Scroll(root.find('#hourScroll')[0], opts.scrollOpt);
        ...
        function createMinute() {
            var minuteDom = root.find('#minuteScroll'),
                minuteHtml = '<li></li>';
            for (var i = 0; i <= 59; i++) {
                if (i < 10) {
            ...
            minuteScroll = new Scroll(root.find('#minuteScroll')[0], opts.scrollOpt);
