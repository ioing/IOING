...
class DOM {
  ...
  creat(root, node, creat) {
    ...
    if (creat && root.quantum.indexOf(root.uuid) == -1) {
  ...
  createTextNode(root, node, value, scope) {
    ...
    text = ['STYLE', 'SCRIPT'].consistOf(node.parentNode.nodeName) ?
      value :
      this.variable(root, scope, value, function() {
  ...
  attributeRule(root, node, name, value, scope) {
    ...
    switch (name) {
      ...
      case 'link':
      case 'path':
      case /^(src:|url:|uri:|link:|href:|path:)+(.*?)$/.test(name) ? name:
        null:
      ...
      case name.indexOf('on-') == 0 ? name:
        null:
          ...
          this.mission[root.order].events.unshift(function(window) {
            ...
            var scopeFunctionEval = function(window) {
              return window.EVALSCOPE[(scopeID ? root.scopeid + ':' : '') + scopeNamedIP] ||
                function(js, e) {
                  new window.Function(scopeFunctionName + '.apply(this, [].slice.call(arguments, 1))')(e)
                }
            }
            var scopeInjectFunction = function(window) {
              return function() {
  ...
  tagRule(root, node, scope, react) {
    ...
    switch (name) {
      ...
      case 'ELSEIF':
        ...
        var origin = this.sign(name, command, 'is', -1, ['(', ')']) ||
          this.sign(name, command, 'not', -1, ['!(', ')']) ||
          command[0].name
        ...
        if (reactEvent.key) {
          ...
          if (scope.getValueByRoute(origin) !== undefined) {
            dom.bind(eventName, function(event) {
              scope.setValueOfHref(origin, name === 'HTMLAREA' ?
                this.textContent :
                this.value ||
      ...
      case 'SCROLL':
        ...
        this.mission[root.order].scroller.push(function(window) {
          ...
          if (finite || infinite) {
            ...
            var turnover = function(start, turnover, callback) {
          ...
          if (finite) {
            config.extend({
              ...
              getFiniteCacheBuffer: function(data, key) {
            ...
            config.extend({
              ...
              getInfiniteDataset: turnover,
              setInfiniteDataFiller: function(nodes, key) {
              ...
              getInfiniteDataFiller: function(tabview, key, rekey) {
      ...
      case 'STYLE':
        ...
        var innerCSSText = function(style, cssText) {
            ...
            if (rooot.scopeid) {
              ...
              if (global) {
                ...
                if (!that.sandbox.window[rooot.spacename]) {
                  ...
                  new that.sandbox.window.Function('__inject__',
                    javascript + '\n'
                    ...
                    +
                    '__inject__["' + rooot.spacename + '"] = new __Global__()'
                ...
                new window.Function('__inject__, shadow, scope, node, global, components, register, root',
                  ...
                  that.sandbox.window[rooot.spacename],
                  rooot.dom,
                  function(name, fn) {
      ...
      case name.indexOf('-') != -1 ? name:
        ...
        var compile = function(source, data, path) {
  ...
  sign(tag, command, sign, index, wrap, boolean, repatriate) {
    ...
    if (index >= 0) {
      ...
      exact = true
      appoint = command.getNamedItem(sign)
      if (!appoint && index === -1) return repatriate
    }

    if (!appoint) this.debug(tag, sign, command)
