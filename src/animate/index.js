...
export const __Animate = () => {
  ...
  class Animate {
    ...
    _transitionend(fn) {
      if (!this.getDuration() || !hasTransitions) return setTimeout(fn, 0)
    ...
    getDuration() {
      if (this.el.transition) return true
      return !!parseFloat(gCS(this.el).transitionDuration, 10)
    }
    ...
    applyProperties() {
      ...
      this.el.css(this._proper.shift())
      this._transitionend(() => {
        this.clear()
        this.next()
      })
