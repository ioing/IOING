...
define('~/application', ['~/proto', '~/source', '~/transform', '~/template'], function (require, module, exports) {
  ...
  var Module = function () {
    ...
    _createClass(Module, [{
      ...
      value: function refresh(dimension, prefetch, readied) {
        ...
        App.transform.update(id, dimension, function (render) {
          if (prefetch) {
            prefetch(render);
          } else {
            render();
      ...
      value: function destroy(type) {
        ...
        if (sandbox) {
          ...
          var cdocument = sandbox.iframe.contentDocument;
    ...
    _createClass(Application, [{
      ...
      value: function get(id, callback, failed) {
        ...
        if (_typeof(modules[id]) !== 'object') {
          ...
          this.take(id, function (uri) {

            // 异步回调重新检测模块是否存在

            if (modules[id] === undefined) {
              modules[id] = new Module(id, require(uri));
      ...
      key: 'take',
      value: function take(id, callback, failed) {
        var that = this;
        var uri = '';
      ...
      key: 'prefetching',
      value: function prefetching(id, param, callback) {
        var _this11 = this,
        ...
        if (modules[id] === undefined) {
          return this.get(id, function () {
            _this11.prefetching.apply(_this11, _arguments3);
          });
        } else {
        ...
        prefetch[id].each(function (i, params) {
          ...
          if (!modules[id].prefetch[params]) {
            ...
            App.source().prefetch(id, modules[id].config, {}.extend(modules[id].initialparam, _this11.filterParam(params)[0]), function () {
      ...
      key: 'prefetch',
      value: function prefetch(id, param) {
        var that = this;
        ...
        function step() {
          requestIdleCallback(function (deadline) {
            ...
            var args = that._prefetching.shift();
      ...
      value: function clearSessionStorage(key, id) {
        try {
          if (key) {
      ...
      value: function clearLocalStorage(key, id) {
        try {
          if (key) {
            (typeof key == 'string' ? [key] : key).each(function (i, key) {
      ...
      key: 'setFileCache',
      value: function setFileCache(id, key, response) {
        var that = this;
        ...
        return new Promise(function (resolve, reject) {
          try {
            if ('caches' in window && (location.protocol === 'https:' || location.hostname === 'localhost')) {
              ...
              localStorage.setItem('[[' + id + ']]:' + key, response);
      ...
      key: 'getFileCache',
      value: function getFileCache(id, key, timeout) {
        var that = this;
      ...
      key: 'clearFileCache',
      value: function clearFileCache(id) {
        if ('caches' in window) {
          if (id) {
            caches.delete(id);
          } else {
