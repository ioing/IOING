...
define('~/scroll', [], function (require, module, exports) {
  ...
  var BROWSER = {
     ...
     * @return {String}
     */
    prefixPointerEvent: function prefixPointerEvent(pointerEvent) {
  ...
  var METHOD = {
     ...
     * @param {Boolean}  capture
     */
    removeEvent: function removeEvent(el, type, fn, capture) {
  ...
  module.exports = function (window, document, Math) {
    ...
    var Scroll = function () {
      ...
      _createClass(Scroll, [{
        ...
        key: 'revert',
        value: function revert(reset, sync) {
          if (!sync) return;
        ...
        key: 'zoom',
        value: function zoom(scale, x, y, time) {
        ...
        key: 'goToPage',
        value: function goToPage(x, y, time, easing) {
          ...
          time = time === undefined ? this.options.snapDuration || Math.max(Math.max(Math.min(Math.abs(posX - this.x), 1000), Math.min(Math.abs(posY - this.y), 1000)), 300) : time;
        ...
        key: 'scrollBy',
        value: function scrollBy(x, y, time, easing, bounce) {
          if (x === 0 && y === 0) return;
        ...
        key: 'scrollTo',
        value: function scrollTo(x, y, time, easing, bounce) {
          time = time || 0;
        ...
        value: function resetInfinite() {
          ...
          this.infiniteElements.each(function (key, element) {
            _this5.setInfiniteDataFiller(element, element._phase);
          });
        ...
        key: 'datasetInfinite',
        value: function datasetInfinite(callback) {
          var _this6 = this;
          ...
          this.getInfiniteDataset.call(this, start, turnover, function (data, end) {
            ...
            */
            ...
            if (!data || !data.length || _this6.turnoverTimes > _this6.options.maxPage) {
              _this6._execEvent('infinitedataend');
              _this6.datasetInfiniteLock = true;

              return;
            }

            _this6.turnoverTimes++;
            ...
            if (!end) {
              ...
              _this6.datasetInfiniteLock = false;
            }

            // infiniteDataLoaded

            _this6._execEvent('infinitedataready');

            // 更新数据缓存

            _this6.updateInfiniteData(start, data);

            // 如果first

            _this6._execEvent('infinitedataloaded');

            // 修改滚动位置

            _this6._execEvent("modify");

            callback && callback.call(_this6);
        ...
        key: 'checkInfiniteDataBuffer',
        value: function checkInfiniteDataBuffer() {
        ...
        key: 'detectInfiniteBalance',
        value: function detectInfiniteBalance(trend) {
        ...
        key: 'reorderInfinite',
        value: function reorderInfinite(callback) {
        ...
        key: 'rearrangeInfinite',
        value: function rearrangeInfinite(supplement, callback) {
          var _this7 = this;
          ...
          var end = function end(_end2) {

            _this7.infiniteRearranging = false;

            // updateInfinitePos

            _this7.updateInfinitePos();

            // callback

            callback && callback.call(_this7);

            if (_end2) {
              _this7._execEvent('infinitecachedend');
            }
          };

          var subStep = function subStep() {
            /*
            	if > 如果缓冲不足 或者 初始化
            ...
            */
            ...
            if (_this7.detectInfiniteBalance(1) >= 0) return end();

            // infiniteSurplusBuffer: 过剩的缓存数据

            _this7.infiniteSurplusBuffer = _this7.infiniteDataLength - _this7.infinitePhase;

            // 如果数据全部展示完毕则停止

            if (_this7.infiniteSurplusBuffer <= 0) return end(1);

            // 当前操作data序列

            phase = _this7.infinitePhase++;

            // 当前操作dom序列

            index = phase % _this7.infiniteLength;

            // 循环序列

            round = Math.max((phase + 1 - _this7.infiniteLength) % _this7.infiniteLength, 0);

            // set item

            item = _this7.infiniteElements[index];
            item._index = index;
            ...
            _this7.updateInfiniteContent(item);

            // content item size

            size = _this7.infiniteItemSize || item._offsetHeight;

            // infinite of content border top and bottom pos

            pos = _this7.infiniteContentPos[1];

            // update item pos

            _this7.promiseInfinitePos(item, pos);

            // mark item pos

            _this7.infiniteElementsPos[index] = [pos, pos + size];

            // update infinite of content border top and bottom pos

            _this7.infiniteElementsEdge = [round, index];
            _this7.infiniteContentPos[0] = _this7.infiniteElementsPos[round][0];
            _this7.infiniteContentPos[1] += size;

            // update maxScrollY and scrollerHeight

            _this7.refreshInfinitePos(phase, pos + size);
          ...
          var supStep = function supStep() {
            ...
            */
            ...
            if (_this7.infinitePhase - _this7.infiniteLength <= 0) return end();
            ...
            round = (phase - 1) % _this7.infiniteLength;
            ...
            item._phase = phase - _this7.infiniteLength;
            ...
            pos = _this7.infiniteContentPos[0] - size;
            ...
            _this7.infiniteElementsEdge = [index, round];
            _this7.infiniteContentPos[0] -= size;
            _this7.infiniteContentPos[1] = _this7.infiniteElementsPos[round][1];

            // update minScrollY and scrollerHeight

            if (item._phase === 0 || pos < _this7.minScrollY) {
              _this7.minScrollY = -pos;
            }
        ...
        key: 'addInfiniteElements',
        value: function addInfiniteElements() {
        ...
        key: 'refreshInfinitePos',
        value: function refreshInfinitePos(phase, pos) {
        ...
        key: 'refreshInfiniteAllPos',
        value: function refreshInfiniteAllPos(start) {
          var i = start || 0,
        ...
        key: 'promiseInfinitePos',
        value: function promiseInfinitePos(item, pos) {
          this.infiniteUpdatePos.push(arguments);
        }
      }, {
        key: 'setInfiniteTranslate',
        value: function setInfiniteTranslate(item, pos) {
        ...
        key: 'updateInfinitePos',
        value: function updateInfinitePos() {
          var _this8 = this;

          // 可插入贞时排布

          rAF(function () {
            _this8.infiniteUpdatePos.each(function (i, args) {
              _this8.setInfiniteTranslate.apply(_this8, args);
            });
            _this8.infiniteUpdatePos = [];
          });
        }
      }, {
        key: 'updateInfiniteContent',
        value: function updateInfiniteContent(item) {
          /**
           * dataFiller
        ...
        key: 'updateInfiniteData',
        value: function updateInfiniteData(start, data, clear) {
          var _this9 = this;

          data.each(function (i, scope) {
            _this9.infiniteCache[start++] = scope;

            // finite 时全部渲染

            if (_this9.options.finite) {
              _this9.getFiniteCacheBuffer.call(_this9, scope, start - 1);
            }
          });
        ...
        key: 'updateInfiniteCache',
        value: function updateInfiniteCache(index) {
          return this.getInfiniteCacheBuffer.call(this, this.infiniteCache[index], index);
        }
      }, {
        key: 'update',
        value: function update(callback, keep) {
          this.updating = true;
          ...
          this.datasetInfinite(function (lock) {
            ...
            if (this.options.finite) {
              ...
              if (lock) return this.updating = false;
            ...
            !lock && callback && callback.call(this);
          });
        }
        ...
        key: '_drawing',
        value: function _drawing(callback) {
          var _this10 = this;

          if (this._drawingKeyFrame) return;

          // mark start

          this._drawingKeyFrame = true;
          ...
          rAF(function () {
            ...
            _this10._drawingKeyFrame = false;

            // drawing

            _this10._drawKeyFrame.call(_this10, callback || noop);
          });
        }
      }, {
        key: '_clearKeyFrame',
        value: function _clearKeyFrame() {
          this._keyFrame = [];
          this._drawingKeyFrame = false;
        }
        ...
        key: '_promiseKeyFrame',
        value: function _promiseKeyFrame(property, value, join) {
          var frame = this._keyFrame;

          // push to queue

          if (join) {
            var tail = frame[frame.length - 1];
          ...
          frame.push(_defineProperty({}, property, value));
        }
      }, {
        key: '_drawKeyFrame',
        value: function _drawKeyFrame(callback) {
        ...
        key: '_translate',
        value: function _translate(x, y, z, s, v) {
          /**
           * 下一贞位置
           * 没有位置数据，设位置为下一位置贞
           */
        ...
        key: '_transformOrigin',
        value: function _transformOrigin(postion) {
        ...
        key: '_transitionScroll',
        value: function _transitionScroll(x, y, time) {
          var _this11 = this;
          ...
          var step = function step() {
            ...
            if (!_this11.isInTransition) return;
            if (Date.now - _this11.transitionStartTime > 2 * _this11.transitionCountTime) return _this11._execEvent('scrollend', 'end');

            var pos = _this11.getComputedPosition();

            // lost rate

            _this11.dropX = Math.round(pos.x - _this11.x) || _this11.dropX || 0;
            _this11.dropY = Math.round(pos.y - _this11.y) || _this11.dropY || 0;

            _this11.x = Math.round(pos.x);
            _this11.y = Math.round(pos.y);

            _this11._execEvent('scroll', 'scrolling');
        ...
        key: '_transitionTimeLeft',
        value: function _transitionTimeLeft() {
          return Date.now() - this.transitionStartTime - this.transitionCountTime;
        }
      }, {
        key: '_transitionEnd',
        value: function _transitionEnd(e) {
          ...
          if (BROWSER.supportTransition && this._checkPerformance() == false) {
        ...
        value: function _animate(destX, destY, duration, easingFn) {
          ...
          var step = function step() {
            ...
            if (timestamp >= destTime) {
              ...
              _this12._translate(destX, destY);

              _this12._checkPerformance();

              if (!_this12.resetPosition(_this12.options.bounceTime)) {
                _this12._execEvent('scrollend', 'end');
              }
            ...
            _this12._translate(Math.round(newX), Math.round(newY));

            if (_this12.isAnimating) rAF(step);

            _this12._execEvent('scroll', 'scrolling');
          };
        ...
        key: '_momentum',
        value: function _momentum(type, time) {
          var current = void 0,
              start = void 0,
              upperMargin = void 0,
              lowerMargin = void 0,
          ...
          speed = speed > .5 ? Math.max(speed, 1.5) : speed;
          deceleration = Math.max(this.options.deceleration - this.acceleration, 0.0006) * DPR;
        ...
        key: '_observer',
        value: function _observer(element, callback) {
          var _this13 = this;
          ...
          element.observer({
            ...
            clearTimeout(_this13.observerTimeout);
            if (records.length) {
              _this13.observerTimeout = setTimeout(function () {
                callback.call(_this13, records);
              }, 1000);
            }
          });
        }
        ...
        key: '_directionLocked',
        value: function _directionLocked(absDistX, absDistY, deltaX, deltaY) {

          // We need to move at least 10 pixels for the scrolling to initiate

          if (this.broken === false && this.moveTime - this.endTime > 200 && absDistX < dP(5) && absDistY < dP(5)) return;
        ...
        value: function _initPrevant() {
          ...
          this.scroller.on('preventscroll', function (e) {
            ...
            _this15.preventscroll = prevent;

            if (prevent) _this15._execEvent('scrollend', 'end');

            if (scroll.options.coverPropagation && _this15.isScrolling()) {
              scroll.stopAnimation = true;
              _this15.preventscroll = false;
            }
          });
        }
      }, {
        key: '_pieceEvent',
        value: function _pieceEvent(e) {
          /**
           * 两次的事件类型检测
           * @type {[type]}
           */
        ...
        key: '_preventscroll',
        value: function _preventscroll(prevent) {
          this.wrapper.trigger('preventscroll', {
            prevent: prevent,
            scroll: this
          });
        }
        ...
        key: '_prevent',
        value: function _prevent(e, order, x, y, ex, ey) {
          var prevent = false;
          ...
          if (this.options.stopPropagation != false) {
            if ([2, 5].consistOf(order)) {
              if (this.options.stopPropagation == true) {
                ...
                if (this.options.stopPropagation == "auto" && (this.canHorizontalScroll && this.directionLocked == 'h' && !(x >= this.minScrollX || x <= this.maxScrollX) || this.canVerticalScroll && this.directionLocked == 'v' && !(y >= this.minScrollY || y <= this.maxScrollY))) {
                  prevent = true;
                }
        ...
        value: function _end(e) {
          ...
          if (this.options.momentum && duration < 300) {
            momentumX = this.hasHorizontalScroll ? this._momentum('x', duration) : {
              destination: newX,
              duration: 0
            };
            momentumY = this.hasVerticalScroll ? this._momentum('y', duration) : {
              destination: newY,
              duration: 0
            };
          ...
          if (this.options.snap) {
            ...
            time = this.options.snapDuration || Math.max(Math.max(Math.min(Math.abs(newX - snap.x), 1000), Math.min(Math.abs(newY - snap.y), 1000)), 300);
            newX = snap.x;
            newY = snap.y;
        ...
        value: function _wheel(e) {
          var wheelDeltaX = void 0,
              wheelDeltaY = void 0,
              absWheelDeltaX = void 0,
              absWheelDeltaY = void 0,
              newX = void 0,
              newY = void 0,
              deltaX = void 0,
              deltaY = void 0,
        ...
        key: '_pulling',
        value: function _pulling(el, r) {
          var u = 1,
              pos = 0,
              upos = 0,
              status = 0,
              style = el.style,
              threshold = -dP(el.getAttribute('threshold') || void 0),
              riseOffset = -dP(el.getAttribute('rise')),
              autoControl = el.attributes.getNamedItem('auto') ? true : false,
              eventTarget = el.getAttribute('event-target') || 'scroll',
              preventDefault = el.getAttribute('prevent-default') || !this.options.finite && !this.options.infinite;
              ...
              if (status == 3 && upos < threshold && (autoControl ? true : type == 'moving')) {
                ...
                if (preventDefault) {
                  this.one('scrollend', function () {
                    ...
                    switch (eventTarget) {
                      case 'module':
                        App.module.refresh(null, function (render) {
                          _this16.one('scrollend', function () {
                            status = 0;
                            el.removeAttribute('pullend');
                            render();
                          });

                          _this16.pulling = false;
                          _this16._execEvent('pullend', null);
                          _this16._refresh();
                          _this16.resetPosition(_this16.options.bounceTime);

                          if (_this16.isBounce() == false) _this16._execEvent('scrollend', null);
                        });
                        break;
                    }
                  });
        ...
        key: '_initKaleidoscope',
        value: function _initKaleidoscope() {
        ...
        value: function _initObserver() {
          ...
          if (!this.options.infinite) {
            this._observer(this.scroller, function (records) {
              ...
              if (this.hasVerticalScroll && this.scroller.offsetHeight !== this.scrollerHeight || this.hasHorizontalScroll && this.scroller.offsetWidth !== this.scrollerWidth) this.refresh();
            });
          }
        }
      }, {
        key: '_indicatorsMap',
        value: function _indicatorsMap(fn) {
          if (this.indicators) {
            ...
            try {
              for (var _iterator2 = this.indicators[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
        ...
        value: function _initIndicators() {
          ...
          try {
            for (var _iterator3 = indicators[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
              var _indicator = _step3.value;

              this.indicators.push(new Indicator(this, _indicator));
            }
        ...
        value: function _initInfinite() {
          ...
          this.one('revert infinitedataloaded', function () {
            _this17._refresh();
            // 异步动画
            rAF(function () {
              _this17._refresh();
          ...
          this.on('scroll scrollend', function (type) {
            if (_this17.borderBouncing) return;
            if (type == "scrolling" && _this17.scrollTrendY === 1 && _this17.transitionCountTime - _this17._transitionTimeLeft() > _this17.transitionCountTime * .6) return;

            _this17.reorderInfinite();
          });

          // 开始infinite >> 获取数据 >> 排序

          this.update(function () {
            if (_this17.scrollcover) {
              _this17.scrollcover.remove();
            }

            // scroll刷新后更新计算数据

            _this17.on('refresh', function () {
              _this17.refreshInfiniteAllPos();
              _this17.reorderInfinite();
            });

            // 监听节点变化后刷新

            _this17.infiniteElements.each(function (i, item) {
              _this17._observer(item, function () {
                if (_this17.infiniteRearranging !== true && item.offsetHeight !== item._offsetHeight) {
                  _this17.refreshInfiniteAllPos();
                  _this17.reorderInfinite();
                }
              });
      ...
      _createClass(Indicator, [{
        ...
        value: function fade(val, hold) {
          ...
          this.fadeTimeout = setTimeout(function () {
            rAF(function () {
              val = val ? '1' : '0';
              _this18.wrapperStyle.opacity = val;
              _this18.visible = +val;
            });
          }, delay);
        }
        ...
        value: function _end(e) {
          ...
          if (scroll.options.snap) {
            ...
            var time = this.options.snapDuration || Math.max(Math.max(Math.min(Math.abs(_x - snap.x), 1000), Math.min(Math.abs(_y - snap.y), 1000)), 300);
