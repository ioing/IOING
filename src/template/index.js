...
export default () => {
  class Template {
    ...
    trick(sandbox, context) {
      ...
      sandbox.window.document.extendProperty("getElementById", (id) => {
        return this.dom.DOM[0][id] || context.find('#' + id)[0] || window.document.getElementById(id)
      })
      sandbox.window.document.extendProperty("getElementsByName", (name) => {
        return context.find('*[name=' + name + ']').toArray() || window.document.getElementsByName(name)
      })
      sandbox.window.document.extendProperty("getElementsByClassName", (name) => {
        return context.find('.' + name).toArray() || window.document.getElementsByClassName(name)
      })
      sandbox.window.document.extendProperty("getElementsByTagName", (name) => {
        return context.find(name).toArray() || window.document.getElementsByTagName(name)
      })
      sandbox.window.document.extendProperty("getElementsByTagNameNS", (name, namespace) => {
    ...
    frame(module) {
      ...
      cwindow.onload = () => {
        ...
        if (cwindow.App) {
          ...
          cwindow.addEventListener(module.config.preview === 2 ? "applicationload" : "applicationready", () => {
            this.readied(module, noop)

            setTimeout(() => {
              this.loaded(module, noop)
            }, 0)
          }, false)
        } else {
          this.readied(module, noop)
