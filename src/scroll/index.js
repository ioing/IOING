...
export const __Scroll = () => {

  const UI = device.ui
  const DPR = UI.scale
  const FEAT = device.feat
  const PREFIX = FEAT.prefixStyle
  const EASEING = {
  ...
  return (window, document, Math) => {
    ...
    class Scroll {
      ...
      getComputedPosition() {
        let matrix = gCS(this.scroller, null),
          x, y
      ...
      scrollBy(x, y, time, easing, bounce) {
        if (x === 0 && y === 0) return
      ...
      scrollTo(x, y, time, easing, bounce) {
        time = time || 0
      ...
      scrollToElement(el, time, offsetX, offsetY, easing) {
        ...
        let elRect = METHOD.getRect(el)
        let wrapperRect = METHOD.getRect(this.wrapper)
      ...
      resetInfinite() {
        ...
        this.infiniteElements.each((key, element) => {
      ...
      datasetInfinite(callback) {
        ...
        let start = this.infiniteCache.length
        let turnover = this.turnoverTimes
        ...
        this.getInfiniteDataset.call(this, start, turnover, (data, end) => {
          ...
          */
          ...
          if ((!data || !data.length) || (this.turnoverTimes > this.options.maxPage)) {
            this._execEvent('infinitedataend')
            this.datasetInfiniteLock = true
          ...
          this._execEvent('infinitedataready')

          // 更新数据缓存

          this.updateInfiniteData(start, data)

          // 如果first

          this._execEvent('infinitedataloaded')
      ...
      rearrangeInfinite(supplement, callback) {

        if (this.infiniteRearranging) return

        let item, pos, size, phase, index, round, i = 0,
        ...
        let end = (end) => {
          ...
          this.infiniteRearranging = false
          ...
          if (end) {
            this._execEvent('infinitecachedend')
          }
        }
        ...
        let subStep = () => {
          ...
          */
          ...
          if (this.detectInfiniteBalance(1) >= 0) return end()
          ...
          if (this.infiniteSurplusBuffer <= 0) return end(1)
          ...
          size = this.infiniteItemSize || item._offsetHeight
          ...
          this.refreshInfinitePos(phase, pos + size)
        ...
        let supStep = () => {
          ...
          */
          ...
          if (this.infinitePhase - this.infiniteLength <= 0) return end()
          ...
          if (item._phase === 0 || pos < this.minScrollY) {
            this.minScrollY = -pos
          }
      ...
      addInfiniteElements() {
        let item = document.createElement('infinite')
      ...
      promiseInfinitePos(item, pos) {
        this.infiniteUpdatePos.push(arguments)
      }

      setInfiniteTranslate(item, pos) {
      ...
      updateInfinitePos() {
        ...
        rAF(() => {
          this.infiniteUpdatePos.each((i, args) => {
            this.setInfiniteTranslate.apply(this, args)
          })
          this.infiniteUpdatePos = []
        })
      }

      updateInfiniteContent(item) {
        /**
         * dataFiller
      ...
      updateInfiniteData(start, data, clear) {
        data.each((i, scope) => {
          this.infiniteCache[start++] = scope

          // finite 时全部渲染

          if (this.options.finite) {
            this.getFiniteCacheBuffer.call(this, scope, start - 1)
          }
        })
      ...
      _refresh(silently) {
        let wrapperRect = METHOD.getRect(this.wrapper)
        let scrollerRect = METHOD.getRect(this.scroller)
        ...
        if ( !silently ) this.resetPosition()
      }
       ...
       */
      _promiseKeyFrame(property, value, join) {
        let frame = this._keyFrame

        // push to queue

        if (join) {
          let tail = frame[frame.length - 1]
      ...
      _drawKeyFrame(callback) {

        // shift queue

        let frame = this._keyFrame.shift()
      ...
      _transitionScroll(x, y, time) {
        ...
        let step = () => {
          ...
          if (Date.now - this.transitionStartTime > 2 * this.transitionCountTime) return this._execEvent('scrollend', 'end')

          let pos = this.getComputedPosition()

          // lost rate

          this.dropX = Math.round(pos.x - this.x) || this.dropX || 0
          this.dropY = Math.round(pos.y - this.y) || this.dropY || 0

          this.x = Math.round(pos.x)
          this.y = Math.round(pos.y)

          this._execEvent('scroll', 'scrolling')
      ...
      _checkPerformance() {
        ...
        if (this.transitionDelayRate > 0.15 && this.transitionCountTime >= 2000) {
          ...
          if (this.transitionDelayRate > 0.5) {
            return false
          }
        }
      ...
      _animate(destX, destY, duration, easingFn) {
        ...
        let step = () => {
          ...
          this._translate(Math.round(newX), Math.round(newY))

          if (this.isAnimating) rAF(step)
       ...
       */
      _momentum(type, time) {
        let current, start, upperMargin, lowerMargin, wrapperSize, speed, defer, distance, distances, direction, duration, destination, deceleration
        ...
        distance = Math.max(speed * speed / deceleration, speed > .5 ? 2000 * speed / defer : 0)
      ...
      _directionLocked(absDistX, absDistY, deltaX, deltaY) {
      ...
      _initPrevant() {
        this.scroller.on('preventscroll', (e) => {
          let scroll = e.data.scroll
          let prevent = e.data.prevent
      ...
      _start(e) {
        ...
        let point = e.touches ? e.touches[e.touches.length - 1] : e
      ...
      _end(e) {
        ...
        let changedTouches = e.changedTouches ? e.changedTouches.length : 0,
            ...
            if (status == 3 && upos < threshold && (autoControl ? true : type == 'moving')) {
              ...
              if (preventDefault) {
                this.one('scrollend', function() {
                  ...
                  switch (eventTarget) {
                    case 'module':
                      App.module.refresh(null, (render) => {
                        this.one('scrollend', function() {
                          status = 0
                          el.removeAttribute('pullend')
                          render()
                        ...
                        if (this.isBounce() == false) this._execEvent('scrollend', null)
                      })
                      break
                  }
                })
       ...
       */
      _initEvents(remove) {
        let bind = remove ? METHOD.removeEvent : METHOD.addEvent
      ...
      _initInfinite() {
        ...
        this.infiniteElements.each((i, item) => {
          item.style["z-index"] = i + 1
        })

        // 默认节点不可见

        this.one('revert infinitedataloaded', () => {
          this._refresh()
          // 异步动画
          rAF(() => {
            this._refresh()
    ...
    class Indicator {
      ...
      refresh() {
        ...
        let r = this.wrapper.offsetHeight // force refresh
      ...
      _end(e) {
        ...
        if (scroll.options.snap) {
          ...
          let snap = scroll._nearestSnap(_x, _y)

          let time = this.options.snapDuration || Math.max(
            Math.max(
